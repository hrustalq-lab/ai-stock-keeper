# Phase 2: Goods Intake with OCR

**Цель:** MVP — приёмка товаров с AI-распознаванием  
**Срок:** 2-3 недели  
**Статус:** ✅ Завершён (все блоки готовы)  
**Начало:** После завершения Phase 1

---

## 1. Обзор

### 1.1 Deliverable
Сканирование товаров → автозаполнение форм → отправка в 1C

### 1.2 User Story
```
Как складской работник,
я хочу сканировать штрих-код или фото накладной,
чтобы автоматически создать приходную накладную в 1C
без ручного ввода данных.
```

### 1.3 Архитектура

```
┌─────────────────────────────────────────────────────────────┐
│                    Intake Flow                               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  📷 Camera/Upload  →  🔍 Recognition  →  🔎 Matching  →  ✅ Confirm  →  📄 1C Doc
│                                                             │
│  ┌──────────────┐   ┌──────────────┐   ┌──────────────┐   ┌──────────────┐
│  │ Quagga.js    │   │ Tesseract.js │   │ Fuse.js      │   │ tRPC API     │
│  │ (barcode)    │   │ (OCR text)   │   │ (fuzzy)      │   │ (1C client)  │
│  └──────────────┘   └──────────────┘   └──────────────┘   └──────────────┘
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. Tech Stack

| Компонент | Библиотека | Версия | Описание |
|-----------|------------|--------|----------|
| **Barcode Scanner** | `@ericblade/quagga2` | ^1.8.x | EAN-13, Code128, Code39, live camera |
| **OCR Engine** | `tesseract.js` | ^5.x | Распознавание текста, поддержка кириллицы |
| **Fuzzy Matching** | `fuse.js` | ^7.x | Нечёткий поиск товаров |
| **UI Components** | `shadcn/ui` | latest | Компоненты интерфейса |
| **Camera Access** | Web API | - | `navigator.mediaDevices.getUserMedia()` |

### 2.1 Почему эти библиотеки

**Quagga.js:**
- Бесплатно, open-source
- Поддержка всех основных форматов штрих-кодов
- Live scanning с камеры
- ~80% accuracy (достаточно для MVP)

**Tesseract.js:**
- Бесплатно, работает в браузере
- Поддержка русского языка (кириллица)
- ~85-90% accuracy
- Можно улучшить до Google Vision API позже

**Fuse.js:**
- Быстрый fuzzy search
- Настраиваемый threshold
- Поддержка кириллицы
- Взвешенный поиск по полям

---

## 3. Задачи

### 3.1 Блок 1: Библиотеки и сервисы (Day 1-2) ✅

| ID | Задача | Файл | Статус |
|----|--------|------|--------|
| 1.1 | Установить зависимости | `package.json` | ✅ |
| 1.2 | Создать BarcodeService | `src/server/services/barcode.ts` | ✅ |
| 1.3 | Создать OCRService | `src/server/services/ocr.ts` | ✅ |
| 1.4 | Создать ProductMatcherService | `src/server/services/product-matcher.ts` | ✅ |
| 1.5 | Unit-тесты для сервисов | `src/__tests__/unit/` | ✅ |

**Зависимости для установки:**
```bash
npm install @ericblade/quagga2 tesseract.js fuse.js
npm install --save-dev @types/quagga
```

---

### 3.2 Блок 2: UI Компоненты (Day 3-5) ✅

| ID | Задача | Файл | Статус |
|----|--------|------|--------|
| 2.1 | BarcodeScanner компонент | `src/app/_components/intake/BarcodeScanner.tsx` | ✅ |
| 2.2 | ImageUploader компонент | `src/app/_components/intake/ImageUploader.tsx` | ✅ |
| 2.3 | ProductSuggestions компонент | `src/app/_components/intake/ProductSuggestions.tsx` | ✅ |
| 2.4 | IntakeForm компонент | `src/app/_components/intake/IntakeForm.tsx` | ✅ |
| 2.5 | IntakeConfirmation компонент | `src/app/_components/intake/IntakeConfirmation.tsx` | ✅ |

---

### 3.3 Блок 3: Backend API (Day 6-7) ✅

| ID | Задача | Endpoint | Статус |
|----|--------|----------|--------|
| 3.1 | Распознать штрих-код | `intake.recognizeBarcode` | ✅ |
| 3.2 | Распознать текст (OCR) | `intake.recognizeOCR` | ✅ |
| 3.3 | Найти товар по тексту | `intake.matchProduct` | ✅ |
| 3.4 | Создать приходную накладную | `intake.createReceipt` | ✅ |
| 3.5 | Получить историю приёмок | `intake.getHistory` | ✅ |
| 3.6 | Получить статистику | `intake.getStats` | ✅ |

**tRPC Router:**
```typescript
// src/server/api/routers/intake.ts
export const intakeRouter = createTRPCRouter({
  recognizeBarcode: publicProcedure
    .input(z.object({ imageBase64: z.string() }))
    .mutation(async ({ input }) => { ... }),

  recognizeOCR: publicProcedure
    .input(z.object({ imageBase64: z.string(), language: z.string().default('rus') }))
    .mutation(async ({ input }) => { ... }),

  matchProduct: publicProcedure
    .input(z.object({ query: z.string(), limit: z.number().default(5) }))
    .query(async ({ input }) => { ... }),

  createReceipt: publicProcedure
    .input(z.object({
      items: z.array(z.object({ sku: z.string(), quantity: z.number() })),
      warehouse: z.string(),
    }))
    .mutation(async ({ input }) => { ... }),

  getHistory: publicProcedure
    .input(z.object({ limit: z.number().default(20), offset: z.number().default(0) }))
    .query(async ({ input }) => { ... }),
});
```

---

### 3.4 Блок 4: Страницы (Day 8-9) ✅

| ID | Задача | Route | Статус |
|----|--------|-------|--------|
| 4.1 | Страница приёмки | `/intake` | ✅ |
| 4.2 | История приёмок | `/intake/history` | ✅ |

**Структура страницы `/intake`:**
```
┌────────────────────────────────────────┐
│  📦 Приёмка товаров                    │
├────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────────┐   │
│  │ 📷 Камера    │  │ 📁 Загрузить │   │
│  │              │  │    файл      │   │
│  └──────────────┘  └──────────────┘   │
├────────────────────────────────────────┤
│  Распознанный код: EAN-4607001234567   │
├────────────────────────────────────────┤
│  Найденные товары:                     │
│  ┌────────────────────────────────┐   │
│  │ ✓ SKU-001 Болт М8x30    (98%)  │   │
│  │   SKU-002 Болт М8x25    (72%)  │   │
│  │   SKU-003 Болт М10x30   (65%)  │   │
│  └────────────────────────────────┘   │
├────────────────────────────────────────┤
│  Количество: [____100____]             │
│  Склад:      [warehouse_main ▼]        │
├────────────────────────────────────────┤
│  [ Отмена ]          [ ✓ Создать ]     │
└────────────────────────────────────────┘
```

---

### 3.5 Блок 5: Тестирование (Day 10) ✅

| ID | Задача | Тип | Статус |
|----|--------|-----|--------|
| 5.1 | Unit-тесты BarcodeService | Jest | ✅ |
| 5.2 | Unit-тесты OCRService | Jest | ✅ |
| 5.3 | Unit-тесты ProductMatcher | Jest | ✅ |
| 5.4 | Integration тест intake flow | Jest | ✅ |
| 5.5 | Manual E2E тестирование | Manual | ⬜ |

**Результаты тестирования:**
- 6 test suites
- 93 tests passed
- Unit-тесты: `barcode.test.ts`, `ocr.test.ts`, `product-matcher.test.ts`
- Integration-тесты: `intake.test.ts` (23 теста для полного intake flow)

---

## 4. Детальные спецификации

### 4.1 BarcodeService

```typescript
// src/server/services/barcode.ts

interface BarcodeResult {
  code: string;
  format: 'ean_13' | 'code_128' | 'code_39' | 'qr_code' | 'unknown';
  confidence: number; // 0-100
}

export class BarcodeService {
  /**
   * Распознать штрих-код из base64 изображения
   */
  async recognize(imageBase64: string): Promise<BarcodeResult | null>;

  /**
   * Настроить параметры сканера
   */
  configure(options: QuaggaConfig): void;
}
```

### 4.2 OCRService

```typescript
// src/server/services/ocr.ts

interface OCRResult {
  text: string;
  confidence: number; // 0-100
  words: Array<{
    text: string;
    confidence: number;
    bbox: { x0: number; y0: number; x1: number; y1: number };
  }>;
}

export class OCRService {
  /**
   * Распознать текст из base64 изображения
   */
  async recognize(imageBase64: string, language?: string): Promise<OCRResult>;

  /**
   * Извлечь структурированные данные (SKU, количество)
   */
  async extractProductData(text: string): Promise<{
    sku?: string;
    name?: string;
    quantity?: number;
  }>;
}
```

### 4.3 ProductMatcherService

```typescript
// src/server/services/product-matcher.ts

interface MatchResult {
  item: {
    sku: string;
    name: string;
    warehouse: string;
  };
  score: number; // 0-1 (1 = perfect match)
  confidence: number; // 0-100%
}

export class ProductMatcherService {
  /**
   * Найти товары по запросу
   */
  async search(query: string, limit?: number): Promise<MatchResult[]>;

  /**
   * Найти товар по точному SKU
   */
  async findBySku(sku: string): Promise<MatchResult | null>;

  /**
   * Обновить индекс товаров из БД
   */
  async refreshIndex(): Promise<void>;
}
```

---

## 5. Таймлайн

```
Week 1 (Days 1-5)
├── Day 1: Установка зависимостей, BarcodeService
├── Day 2: OCRService, ProductMatcherService
├── Day 3: BarcodeScanner UI, ImageUploader UI
├── Day 4: ProductSuggestions UI, IntakeForm UI
└── Day 5: Интеграция компонентов

Week 2 (Days 6-10)
├── Day 6: tRPC Router (intake)
├── Day 7: API интеграция с UI
├── Day 8: Страница /intake
├── Day 9: Страница /intake/history, UX polish
└── Day 10: Тестирование, документация
```

---

## 6. Файловая структура

```
src/
├── app/
│   ├── _components/
│   │   └── intake/
│   │       ├── BarcodeScanner.tsx
│   │       ├── ImageUploader.tsx
│   │       ├── ProductSuggestions.tsx
│   │       ├── IntakeForm.tsx
│   │       └── IntakeConfirmation.tsx
│   └── intake/
│       ├── page.tsx
│       └── history/
│           └── page.tsx
├── server/
│   ├── api/
│   │   └── routers/
│   │       └── intake.ts
│   └── services/
│       ├── barcode.ts
│       ├── ocr.ts
│       └── product-matcher.ts
└── __tests__/
    └── unit/
        ├── barcode.test.ts
        ├── ocr.test.ts
        └── product-matcher.test.ts
```

---

## 7. Критерии успеха

| Метрика | Цель | Измерение |
|---------|------|-----------|
| **Barcode accuracy** | >80% | % успешных распознаваний |
| **OCR accuracy** | >85% | % корректно извлечённых SKU |
| **Product match rate** | >90% | % правильных совпадений в top-3 |
| **Intake time** | <30 sec | Время от скана до отправки в 1C |
| **Error rate** | <5% | % неуспешных приёмок |

---

## 8. Риски и митигации

| Риск | Вероятность | Влияние | Митигация |
|------|-------------|---------|-----------|
| Низкая accuracy OCR | Средняя | Высокое | Manual override, обучение на данных |
| Проблемы с камерой в браузере | Средняя | Среднее | Fallback на загрузку файла |
| Медленный Tesseract.js | Высокая | Среднее | Web Worker, loading indicator |
| Нет товара в БД | Низкая | Низкое | UI для добавления нового товара |

---

## 9. Зависимости от Phase 1

| Компонент Phase 1 | Использование в Phase 2 |
|-------------------|-------------------------|
| `OneCClient` | Создание приходных накладных |
| `InventoryCache` | Поиск товаров для matching |
| `db.inventory` | Индекс для Fuse.js |
| tRPC setup | Новый router `intake` |

---

## 10. Следующие шаги

После завершения Phase 2:
- [x] **Phase 3:** Real-Time Inventory Sync (Dashboard) — [PHASE-3.md](./PHASE-3.md)
- [ ] **Phase 4:** Predictive Analytics
- [ ] **Future:** Google Vision API для production OCR
- [ ] **Future:** ML model для улучшения matching

---

**Документ создан:** 2026-01-31  
**Последнее обновление:** 2026-01-31  
**Автор:** AI Stock Keeper Team
